...
class MusicController {
    ...
    public Map<String, Object> purchaseMusic(@RequestParam("musicId") int musicId,
        ...
        Map<String, Object> result = new HashMap<>();
        int resultCode=purchaseMusicService.addPurchaseMusic(musicId,userId);
    ...
    public Map<String, Object> getPurchaseStatus(@RequestParam("musicId") int musicId,
        ...
        if(musician!=null&&music.musicianId==musician.musicianId){
            result.put("status","true");
    ...
    public Map<String, Object> uploadAppendix(@RequestParam("file") MultipartFile file) {
        ...
        try {
            File path = new File(ResourceUtils.getURL("classpath:").getPath());
            System.out.println(path.getAbsolutePath());
            File upload = new File(path.getAbsolutePath(), "static/musicAppendix/");
            if (!upload.exists()) upload.mkdirs();
            String uploadPath = upload + "/";
            String uuid = UUID.randomUUID().toString().replaceAll("-", "");
